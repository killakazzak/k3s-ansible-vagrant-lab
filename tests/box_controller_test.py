...
for controller in ['IDE Controller','IDE']:
 with tempfile.TemporaryDirectory() as d:
  ...
  assert '(var.os_arch == "aarch64" ? "IDE" : "IDE Controller")' in first
  subprocess.run([sys.executable,str(script),d],check=True)
  assert source.read_text()==first
print('Pristine and previously patched controllers: architecture selection and repeatability passed')
