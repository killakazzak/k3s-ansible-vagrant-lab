...
LOCK = threading.Lock()
JOB = None
LAB_ACTIONS = {'rancher_install','rabbit_plugins','app_start','app_stop','app_restart','app_delete','app_deploy','template_deploy','app_update','app_rollback','app_check','stand_stop','stand_start'}
        ...
        try:
            ...
            if self.path == '/api/templates':
                if data.get('operation') in ('storage-options','check','check-storage','suggest-name','editor-storage'):
                    ...
                    if data['operation']=='editor-storage':return self.reply(200,template_storage.editor_storage(apps,data))
                    if data['operation']=='suggest-name':return self.reply(200,template_storage.suggest_name(apps,data))
