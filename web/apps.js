...
function closeTemplateMenu(focus=false){$('template-options').hidden=true;$('template-toggle').setAttribute('aria-expanded','false');if(focus)$('template-toggle').focus()}
function renderTemplateMenu(){
 const current=savedTemplates.find(t=>t.name===$('template-list').value)||savedTemplates[0];$('template-list').value=current?.name||'';$('template-value').textContent=current?templateCaption(current):'Выберите шаблон';$('template-options').replaceChildren();
 for(const t of savedTemplates){const b=element('button',undefined,'template-option');b.type='button';b.setAttribute('aria-pressed',String(t.name===current?.name));b.append(element('strong',t.name),element('small',t.apps.map(a=>a.name).join(' · ')));b.onclick=()=>{$('template-list').value=t.name;renderTemplateMenu();closeTemplateMenu(true)};$('template-options').append(b)}
}
$('template-toggle').onclick=()=>{const open=$('template-options').hidden;$('template-options').hidden=!open;$('template-toggle').setAttribute('aria-expanded',String(open));if(open)$('template-options').querySelector('[aria-pressed="true"]')?.focus()};
document.addEventListener('click',e=>{if(!e.target.closest('.template-dropdown'))closeTemplateMenu()});
$('template-toggle').onkeydown=e=>{if(e.key==='ArrowDown'){e.preventDefault();if($('template-options').hidden)$('template-toggle').click()}};
$('template-options').onkeydown=e=>{const buttons=[...$('template-options').querySelectorAll('button')];const i=buttons.indexOf(document.activeElement);if(e.key==='Escape'){e.preventDefault();closeTemplateMenu(true)}else if(['ArrowDown','ArrowUp','Home','End'].includes(e.key)){e.preventDefault();const next=e.key==='Home'?0:e.key==='End'?buttons.length-1:(i+(e.key==='ArrowDown'?1:-1)+buttons.length)%buttons.length;buttons[next]?.focus()}};
document.addEventListener('focusin',e=>{if(!e.target.closest('.template-dropdown'))closeTemplateMenu()});
async function loadTemplates(){try{const values=await api('templates');const changed=JSON.stringify(values)!==JSON.stringify(savedTemplates);savedTemplates=values;$('template-picker').hidden=!savedTemplates.length;$('template-hint').textContent=savedTemplates.length?'Готовые наборы приложений для выбранного кластера.':'Сохраните набор приложений в каталоге — он появится здесь.';if(changed||!$('template-value').textContent){closeTemplateMenu();renderTemplateMenu()}}catch(e){$('apps-message').textContent=e.message}}
