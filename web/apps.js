...
const catalogDefaults={nginx:{image:'nginx:1.30.4-alpine',port:80,memory:128},postgres:{image:'postgres:17-alpine',port:5432,memory:256},redis:{image:'redis:7.4-alpine',port:6379,memory:128},custom:{image:'',port:8080,memory:128}};
...
function openCatalog(){
 ...
 const type=labField('type','Приложение','nginx','text',['nginx','postgres','redis','custom']);
 labField('name','Имя приложения','web');labField('namespace','Namespace','dev');labField('image','Образ с версией',catalogDefaults.nginx.image);labField('port','Порт контейнера',80,'number');labField('replicas','Реплики',1,'number');labField('cpu','Запрос CPU, millicores (1000 = 1 ядро)',100,'number');labField('memory','Запрос RAM, MiB',128,'number');labField('storage','Диск для базы, GiB',2,'number');const host=labField('host','URL: префикс (web) или полный hostname','web');host.required=false;
 type.onchange=()=>{const t=type.value,d=catalogDefaults[t];$('lab-image').value=d.image;$('lab-port').value=d.port;$('lab-memory').value=d.memory;$('lab-name').value=t==='custom'?'api':t;const db=['postgres','redis'].includes(t);$('lab-replicas').value=1;$('lab-replicas').max=db?'1':'10';$('lab-host').value=db?'':t;};
 const note=element('p','Лимиты CPU/RAM = 2 × запрос. PostgreSQL и Redis — по одной реплике; увеличение числа реплик само по себе не создаёт репликацию базы.','lab-note');$('lab-extra').append(note);
...
function standAction(action){const sum=state.nodes.reduce((a,n)=>({cpu:a.cpu+n.cpu,ram:a.ram+n.ram,disk:a.disk+n.disk}),{cpu:0,ram:0,disk:0});labOpen(action==='stand_stop'?'Остановить VM стенда':'Возобновить VM стенда',`Кластер ${selectedCluster}: ${state.nodes.length} VM, ${sum.cpu} CPU, ${(sum.ram/1024).toFixed(1)} GiB RAM, ${sum.disk} GiB дисков. `+(action==='stand_stop'?'VM будут штатно выключены. Диски, Kubernetes и данные сохранятся.':'Будут запущены существующие VM без повторного развёртывания.'),()=>labAction(action,{}));}
