...
function bulkLifecycle(action,items=[...selectedApps.values()]){if(!items.length)return;const subject=items.length===1?items[0].name:'выделенные приложения';labOpen((action==='app_stop'?'Остановить ':'Перезапустить ')+subject+'?',action==='app_stop'?'Приложения и их веб-панели будут уменьшены до 0 реплик. Данные сохранятся. Для запуска нажмите «Перезапустить».':'Pod приложений и веб-панелей будут перезапущены последовательно. Возможен перерыв в доступности. Остановленные приложения запустятся с прежним числом реплик (если оно не сохранено — с одной репликой).',async()=>{sameCluster();await api('action',{action,confirmed:true,params:{apps:items.map(a=>({kind:a.kind,name:a.name,namespace:a.namespace}))}});$('lab-dialog').close();await poll()});for(const a of items)$('lab-extra').append(element('p',a.namespace+'/'+a.name+' · '+a.kind,'lab-note'))}
