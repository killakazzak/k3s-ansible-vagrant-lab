...
function addStorageFields(type){
 ...
 const note=element('p','PVC доступен только приложению в том же namespace. Новый PVC создаётся в namespace приложения. Существующий PVC подключается без очистки. Версия и формат данных должны соответствовать приложению. Для PostgreSQL/RabbitMQ нужны прежний пароль и пользователь app.','lab-note');$('lab-extra').append(note);
