...
async function closePodDebug(){const s=podDebugSession;podDebugSession=null;if(!s)return;s.closed=true;s.observer?.disconnect();clearTimeout(s.resizeTimer);s.term?.dispose();try{await terminalApi(s.cluster,{operation:'close',id:s.id})}catch(e){}}
...
async function openPodDebug(pod,options={}){
 ...
 const session={id:result.id,cluster,closed:false,term:null};podDebugSession=session;stop.disabled=false;
