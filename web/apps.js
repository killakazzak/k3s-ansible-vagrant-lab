...
let pvcData=[];
async function refreshPVCs(){const cluster=selectedCluster;try{const data=await api('pvcs');if(cluster!==selectedCluster)return;pvcData=data;const box=$('pvc-list');box.replaceChildren();for(const p of data){const row=element('article',undefined,'app-card');row.append(element('h3',p.namespace+'/'+p.name),element('p',p.phase+' · '+p.capacity+' · '+p.accessModes.join(', ')),element('p','StorageClass: '+(p.storageClass||'—')),element('p','PV: '+(p.volume||'не назначен')),element('p','Pod: '+(p.pods.join(', ')||'не используется')));box.append(row)}if(!data.length)box.append(element('p','В кластере пока нет PVC.'));}catch(e){if(cluster===selectedCluster)$('pvc-list').textContent='PVC недоступны: '+e.message}}
function addStorageFields(type){
 ...
 const note=element('p','Существующий PVC подключается без очистки. Версия и формат данных должны соответствовать приложению. Для PostgreSQL/RabbitMQ нужны прежний пароль и пользователь app.','lab-note');$('lab-extra').append(note);
 const update=()=>{const db=['postgres','redis','kafka','rabbitmq'].includes(type.value);if(db&&mode.value==='none')mode.value='new';claim.parentElement.hidden=mode.value!=='existing';claim.required=mode.value==='existing';$('lab-storage').parentElement.hidden=mode.value!=='new';mount.parentElement.hidden=db||mode.value==='none';secret.parentElement.hidden=mode.value!=='existing'||!['postgres','rabbitmq'].includes(type.value);secret.required=!secret.parentElement.hidden;const old=claim.value;claim.replaceChildren();const blank=element('option','Выберите PVC');blank.value='';claim.append(blank);for(const p of pvcData.filter(p=>p.namespace===$('lab-namespace').value)){const o=element('option',p.name+' · '+p.capacity+' · '+p.phase+(p.pods.length?' · занят':''));o.value=p.name;o.disabled=p.phase!=='Bound'||p.pods.length>0;claim.append(o)}if([...claim.options].some(o=>o.value===old))claim.value=old;};
