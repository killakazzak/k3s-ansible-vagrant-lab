...
function renderResourceDetail(item){
 ...
 const actions=element('div',undefined,'pod-detail-actions');if(!state?.external)actions.append(resourceButton('>_ Терминал',()=>openPodTerminal(item).catch(labError),'button primary'));actions.append(resourceButton('Онлайн-дебаг',()=>openPodDebug(item),'button primary'),resourceButton('Логи и события',()=>showPodDiagnostics(item),'button secondary'));body.prepend(actions);
...
function podTableHead(pods,filter){
 ...
 for(const [label,sortKey,filterKey,values] of columns){
  ...
  if(sortKey){
   ...
   const button=resourceButton('',()=>{filter.podDirection=active?(filter.podDirection==='desc'?'asc':'desc'):(['cpu','memory'].includes(sortKey)?'desc':'asc');filter.podSort=sortKey;renderResources(true)},'column-sort');
 ...
 row.append(element('th','Действия'));head.append(row);return head;
}
