...
function renderResources(force=false){
 ...
 for(const [type,label] of [['pods','Pods'],['pvcs','PVC'],['secrets','Secrets']]){const tab=$('resource-tab-'+type);tab.replaceChildren(element('span',label),element('span',String(sets[type].filter(p=>!filter.namespace||p.namespace===filter.namespace).length),'resource-tab-count'));tab.setAttribute('aria-selected',String(type===resourceTab));tab.tabIndex=type===resourceTab?0:-1;}
 ...
 if(!items.length){content.append(element('p','Ресурсов по выбранным фильтрам нет.','resource-empty'));if(errorText&&/недоступ|ошиб|token|отказ/i.test(errorText))content.append(element('p',errorText,'error'));}
  ...
  for(const p of items){const row=document.createElement('tr');if(resourceSelected?.cluster===selectedCluster&&resourceSelected.type===resourceTab&&resourceSelected.key===resourceKey(p))row.className='selected';const name=document.createElement('td');const nameButton=resourceButton('',()=>showResource(resourceTab,p),'resource-name');const icon=element('span',{pods:'◇',pvcs:'▤',secrets:'⌘'}[resourceTab],'resource-type-icon');icon.setAttribute('aria-hidden','true');nameButton.append(icon,element('span',p.name));name.append(nameButton);row.append(name);const values=resourceTab==='pods'?[p.namespace,p.ready?'Ready':p.phase,p.node||'Не назначен',String((p.statuses||[]).reduce((a,c)=>a+c.restarts,0))]:resourceTab==='pvcs'?[p.namespace,p.capacity,p.phase,p.location?.server||(p.location?.nodes||[]).map(n=>n.name).join(', ')||'—']:[p.namespace,p.type,p.keys.join(', ')];for(const [index,value] of values.entries()){const td=element('td');const isStatus=(resourceTab==='pods'&&index===1)||(resourceTab==='pvcs'&&index===2);if(index===0)td.append(element('span',value,'resource-ns-chip'));else if(isStatus){const tone=['Ready','Bound','Succeeded'].includes(value)?'good':['Pending','Running'].includes(value)?'warn':'bad';td.append(element('span',value,'resource-status-chip '+tone))}else td.append(element('span',value));row.append(td)}row.onclick=e=>{if(!e.target.closest('button'))showResource(resourceTab,p)};tbody.append(row)}table.append(tbody);content.append(table);
 }
