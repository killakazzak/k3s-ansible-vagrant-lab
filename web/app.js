...
function render() {
  ...
  for (const node of state.nodes) {
    ...
    const status=element('td');status.append(element('span',node.state,'badge '+node.state));tr.append(status,element('td',node.ip),element('td',node.cpu+' CPU / '+node.ram/1024+' ГБ / '+node.disk+' ГБ диск'));
