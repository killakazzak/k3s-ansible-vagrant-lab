...
function versionPicker(current){
 const label=element('label','Версия k3s');label.htmlFor='field-version';
 const select=element('select');select.id='field-version';select.name='version';select.required=true;
 const note=element('p','Сейчас установлена: '+current);note.setAttribute('role','status');
 const retry=element('button','Повторить загрузку','button secondary');retry.type='button';retry.hidden=true;
 $('fields').append(label,select,note,retry);
 const load=async()=>{
  select.replaceChildren();const pending=element('option','Загрузка последних версий…');pending.value='';select.append(pending);select.disabled=false;retry.hidden=true;
  try{const result=await api('k3s-versions');if(!select.isConnected)return;
   select.replaceChildren();for(const v of result.versions){const option=element('option',v+(v===current?' · установлена сейчас':''));option.value=v;select.append(option)}
   select.value=result.versions.includes(current)?current:result.versions[0];note.textContent='Сейчас установлена: '+current+'. Пять последних стабильных версий k3s из GitHub.';
  }catch(e){if(!select.isConnected)return;pending.textContent='Не удалось загрузить версии';note.textContent=e.message;retry.hidden=false}
 };
...
function openAction(action,nodeName){if(!state)return;currentAction=action;$('fields').replaceChildren();$('form-error').hidden=true;$('submit').hidden=false;$('submit').textContent='Подтвердить';$('modal-title').textContent=definitions[action][0];$('modal-description').textContent='Кластер: '+selectedCluster+'. '+definitions[action][1];
 ...
 if(action==='version')versionPicker(state.version);
