...
function error(message) {
  ...
  $('error').textContent = pending
    ? (creating ? 'Кластер создаётся. Доступ появится после завершения настройки. Ход развёртывания — в журнале ниже.' : 'Кластер пока не готов к подключению. Если создание уже запущено, дождитесь завершения. Иначе нажмите «Изменить».')
    : message;
...
function render() {
  ...
  if(!state.nodes.length){const row=element('tr'),cell=element('td','Кластер пуст. Нажмите «Изменить».');cell.colSpan=5;row.append(cell);$('node-list').append(row)}
