#!/bin/bash
# Install the pinned official Vagrant package bundled in this checkout.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
export PATH="$PATH:/opt/vagrant/bin:/usr/local/bin"
if [[ "${1:-}" == --help ]]; then
  echo 'Usage: ./scripts/install-vagrant.sh [--check]'
  echo 'Install bundled Vagrant if absent. --check verifies the DMG/package without installing.'
  exit 0
fi
[[ $# -eq 0 || ( $# -eq 1 && "$1" == --check ) ]] || { echo 'Unknown option; use --help.' >&2; exit 2; }
[[ "$(uname -s)" == Darwin && "$(uname -m)" == arm64 ]] || { echo 'Bundled installer supports Apple Silicon Macs only.' >&2; exit 1; }
...
dmg="$ROOT/vendor/vagrant/vagrant_${version}_darwin_arm64.dmg"
[[ -f "$dmg" ]] || { echo "Bundled installer missing: $dmg. Restore vendor/vagrant from this repository." >&2; exit 1; }
...
echo "Installing Vagrant $version from this repository. macOS may request your administrator password."
if [[ $EUID -eq 0 ]]; then
