...
- Распространяется через тот же GitHub Release `vagrant-2.4.9`.
...
Для подготовки офлайн перенесите DEB в `.cache/installers/` и выполните `./scripts/install-vagrant.sh --check` на Ubuntu. Проверка не устанавливает пакет. Зависимости apt должны быть доступны отдельно; этот DEB не заменяет полный офлайн-набор платформы.
